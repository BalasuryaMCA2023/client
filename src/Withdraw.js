...
export default function Withdraw(){
    ...
    useEffect(()=>{
        const fetchdata=async()=>{
           await axios.get('https://bank-server-2.onrender.com/data').then((item)=>{setData(item.data)})
        };fetchdata()
     },[]);
       ...
       for(let i=0;i<data.length;i++){
            if(data[i].id === Number(userId)){
                if(data[i].amount>0){
                   ...
                   let url= ("https://bank-server-2.onrender.com${data[i]._id ") 
    ...
    <Form  onSubmit={handleClick}  >
      <h4 id="Heading">Withdraw Your Amount Here</h4><hr></hr>
      <Form.Group className="mb-3" controlId="formBasicEmail">
        <Form.Label id="label"  >Account No:</Form.Label>
        <Form.Control type="number"  id="input"  required onChange={(e)=>{setUserId(e.target.value)}}  />
        <Form.Label id="label" >Withdraw Amount:</Form.Label>
        <Form.Control type="number" id="input" required onChange={(e)=>{setWithdraw(e.target.value)}}/>
